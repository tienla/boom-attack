...
#define ROUNDS 2 // run the train + attack sequence X amount of times (for redundancy)
...
void victimFunc(uint64_t idx){
    ...
    //printf("idx1:%lu\n",idx);
    if (idx < array1_sz){
        predicate_state = !(idx < array1_sz) ? all_zeros_mask : predicate_state;
        idx &= predicate_state;
        //printf("idx2:%lu\n",idx);
...
int main(void){
    ...
    uint64_t start, diff, passInIdx, randIdx, start1,diff1;
    uint8_t dummy = 0;
